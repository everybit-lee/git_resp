# -*- coding: utf-8 -*-
"""
Created on 20171122 用pandas读取excel中的矩阵数据，并用kmeans进行聚类
聚类过程由以下几步：
1、从excel中读取数据，
2、画出原始图形
3、用肘部法确定K值拐点
4、计算肘部附近的K值的轮廓系数,确定最佳的K值
5、用最佳K值进行聚类，并解释结果
...
for st in filerange:
    ...
    pngfile =pngfile+ "zhou" + str(st) + ".png"
    plt.savefig(pngfile, dpi=120)
    #plt.show()
    pngfile = "..\\data\\journals\\results\\"
    ...
    #     print kmeans_model.labels_:每个点对应的标签值
        for i,l in enumerate(kmeans_model.labels_):
            ...
            plt.title(u'K = %s, 轮廓系数 = %.03f' %
                      ...
                      ,fontproperties=font)
    pngfile =pngfile+ "res" + str(st) + ".png"
    plt.savefig(pngfile, dpi=120)
    #plt.show()
